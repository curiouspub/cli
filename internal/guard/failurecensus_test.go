...
func TestTheCheckerVisitsEveryConstruction(t *testing.T) {
	...
	for _, path := range publishedTextFiles(t, root) {
		...
		// helperNames are the constructors and forwarding helpers whose
		// own bodies are DEFINITIONS rather than constructions.
		helperNames := map[string]bool{
			"NewFailure": true, "Quoted": true, "uploadFailed": true,
		}
		...
		ast.Inspect(f, func(n ast.Node) bool {
			...
			case *ast.CompositeLit:
				...
				if !helperNames[enclosingFunc(f, node.Pos())] {
