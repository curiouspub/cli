...
var repoExceptions = []repoException{
	{
		...
		stillTrue: func(t *testing.T, root string) (string, bool) {
			t.Helper()
			cmd := exec.Command("git", "ls-files", "-z", "--", "vendor")
			cmd.Dir = root
			out, err := cmd.Output()
			if err != nil {
			...
			if listed == 0 {
				return "vendor/ holds no tracked file, so nothing is excluded by it", true
			}
...
func TestTheEnumerationAndAWalkDisagreeInBothDirections(t *testing.T) {
	...
	if strings.Join(walked, ",") == strings.Join(got, ",") {
		t.Error("the walk and the enumeration returned the same set, so this row is " +
			"measuring nothing. They disagree in both directions by construction: " +
			"one file is ignored and one is deleted.")
	}
}
