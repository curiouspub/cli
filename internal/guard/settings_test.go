...
func failureParts(n ast.Node, text func(ast.Node) string) (whole, why, kind string, ok bool) {
	...
	case *ast.CallExpr:
		...
		switch name {
		case "NewFailure":
			// THE WHY IS ARGUMENT FOUR. It moved to three when the
			// constructor gained a family id, and to four when it gained a
			// declared stage. This guard reddened both days, which is the
			// behaviour to keep: a positional summary that is wrong about a
			// signature must fail rather than read a neighbouring argument
			// as the one it wanted — the second time, it read the headline
			// as the Why and reported a setting missing from a paragraph
			// that names it.
			if len(node.Args) < 4 {
				return "", "", "", false
			}
			return text(node), text(node.Args[3]), "NewFailure", true
		case "Quoted":
