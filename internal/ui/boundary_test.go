...
func renderers() []aRenderer {
	return []aRenderer{
		...
		{"Fail", func(t *testing.T, text string) string {
			u, out, errOut := testUI("", false, nil)
			u.Fail(NewFailure(IDInternalFault, StageThisRun, text, text, NextGiveUp, text))
			return bothStreams(out, errOut)
		}},
...
func TestAFailureKeepsItsParagraphs(t *testing.T) {
	...
	u.Fail(NewFailure(IDInternalFault, StageThisRun, "What.", "First line.\n\nSecond paragraph.", NextGiveUp, "Do this."))
	got := errOut.String()
...
func TestAQuotedSentenceCannotAddALineOfItsOwn(t *testing.T) {
	...
	u, _, errOut := testUI("", false, nil)
	u.Fail(NewFailure(IDInternalFault, StageThisRun, "The build finished, and the server would not take the result.",
...
func TestAParagraphThatMerelyREADSLikeTheQuotationKeepsItsLayout(t *testing.T) {
	...
	u, _, errOut := testUI("", false, nil)
	u.Fail(NewFailure(IDInternalFault, StageThisRun, "What.", same, NextGiveUp, "Next.").Quoting(same))
	got := errOut.String()
...
func TestAValueInThisProgramsProseCannotAddAParagraph(t *testing.T) {
	...
	u, _, errOut := testUI("", false, nil)
	u.Fail(NewFailure(IDInternalFault, StageThisRun, "What.", why, NextGiveUp, "Next."))
	got := errOut.String()
