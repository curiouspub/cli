...
func TestExitCode(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			err:        NewFailure(IDInternalFault, StageThisRun, "Something specific went wrong.", "Because of this.", NextGiveUp, "Do that."),
			wantCode:   1,
		...
		},
		{
			...
			err:        ServerClosed(NewFailure(IDInternalFault, StageThisRun, "We're full for today.", "Because of this.", NextGiveUp, "Come back at 12:15.")),
			wantCode:   ExitServerClosed,
		...
		},
		{
			...
			err:       fmt.Errorf("verifying the code: %w", ServerClosed(NewFailure(IDInternalFault, StageThisRun, "Full.", "Why.", NextGiveUp, "Next."))),
			wantCode:  ExitServerClosed,
			wantOnErr: "Full.",
		},
		{
			...
			err:        NewFailure(IDInternalFault, StageThisRun, "Too many requests from here.", "Because of this.", NextGiveUp, "Try later."),
			wantCode:   1,
			wantAbsent: internalWhat,
